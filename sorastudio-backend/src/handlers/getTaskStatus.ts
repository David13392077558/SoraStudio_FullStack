...
export const getTaskStatusHandler = async (req: Request, res: Response) => {
  try {
    ...
    // 先检查缓存
    const cachedResult = await cacheService.getCachedTaskResult(taskId);
    if (cachedResult) {
    ...
    if (taskResults[taskId]) {
      // 缓存结果
      await cacheService.cacheTaskResult(taskId, taskResults[taskId]);
      return res.json(taskResults[taskId]);
    }

    // 检查队列中的任务
    const jobs = await taskQueue.getJobs(['active', 'waiting', 'completed', 'failed']);
    ...
    if (job.opts.delay) {
      ...
      await cacheService.cacheTaskResult(taskId, finalResult);
...
export const setTaskResult = (taskId: string, result: any) => {
  taskResults[taskId] = result;
  // 异步缓存，不阻塞
  cacheService.cacheTaskResult(taskId, result).catch(err =>

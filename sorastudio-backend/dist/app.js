...
const corsOptions: cors.CorsOptions = {
  origin(origin, callback) {
    ...
    /**
     * 自动放行所有 Vercel Preview 域名
     * 规则：
     *   - 必须包含项目名：sorastudio-frontend-v2
     *   - 必须以 .vercel.app 结尾
     *   - 中间 hash / 团队空间 / 分支名随便变
     */
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  setInterval(() => {
    ...
    console.log(
      `📊 内存: ${(memory.heapUsed / 1024 / 1024).toFixed(
        2
      )}MB / ${(memory.heapTotal / 1024 / 1024).toFixed(
        2
      )}MB (文件缓冲数: ${fileBuffers.size})`
    );
  }, 30000);
});
